...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="${entityName?uncap_first}List"<#if cgformConfig.cgFormHead.isCheckbox == 'Y'> checkbox="true"<#else> checkbox="false"</#if><#if cgformConfig.cgFormHead.isPagination == 'Y'> pagination="true"<#else> pagination="false"</#if><#if cgformConfig.cgFormHead.isTree == 'Y'> treegrid="true" treeField="${cgformConfig.cgFormHead.treeFieldnamePage}"</#if> fitColumns="true" title="${ftl_description}" <#if orderByCreateDate == true >sortName="createDate"<#else>sortName="id"</#if> actionUrl="${entityName?uncap_first}Controller.do?datagrid" idField="id" fit="true" queryMode="group">
...
<#else>
 ...
 />
   ...
   <t:dgToolBar title="编辑" icon="icon-edit" url="${entityName?uncap_first}Controller.do?goUpdate" funname="updatetree"></t:dgToolBar>
   ...
   <t:dgToolBar title="查看" icon="icon-search" url="${entityName?uncap_first}Controller.do?goUpdate" funname="detailtree" ></t:dgToolBar>
   <#else>
