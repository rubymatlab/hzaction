...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding: 0px; border: 0px">
		<t:datagrid name="vwBusPoApplyList" checkbox="true" fitColumns="true"
			...
			actionUrl="vwBusPoApplyController.do?datagrid&state=0" idField="id"
			...
			<t:dgCol title="客户名称" field="bcName" hidden="true" query="true"
				...
				dictionary="msg_customerName,bcName,bc_name" popup="true"></t:dgCol>
			<t:dgCol title="项目经理" field="bpManager" hidden="true" query="true"
				dictionary="msg_managerName,bpManager,bp_manager" popup="true"
				queryMode="single" width="120"></t:dgCol>
			...
			<t:dgDelOpt title="删除" url="vwBusPoApplyController.do?doDel&id={id}"
				urlclass="ace_button" urlfont="fa-trash-o" />
			<t:dgToolBar title="新增" icon="icon-add"
				url="vwBusPoApplyController.do?goAdd" funname="add" width="100%"
				height="100%"></t:dgToolBar>
			<t:dgToolBar title="修改" icon="icon-edit"
				url="vwBusPoApplyController.do?goUpdate" funname="update"
			...
			<t:dgToolBar title="送审" icon="icon-tip"
				url="vwBusPoApplyController.do?doBtn_submit" funname="doBtn_submit"></t:dgToolBar>
		</t:datagrid>
	</div>
</div>
...
<script type="text/javascript">
	...
	function doBtn_submit(title, url, id) {
		...
		$.dialog.confirm('你确定要送审项目吗?', function(r) {
			if (r) {
				$.ajax({
					url : url,
					type : 'post',
